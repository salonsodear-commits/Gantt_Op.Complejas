...
def build_all():
    ...
    heads=["Fila","EsFase","EsTarea","Módulo","SprintRaw","Tarea","Responsable","FechaObj","Progreso",
           ...
           "Motivo","ClaveAud","ClavePri","urg","atr","rsk","AlertRaw","CierrePlan","DesvíoSprint"]
    ...
    for r in range(DATA_FIRST, DATA_LAST+1):
        ...
        d.f(r,34, f'IF($AF{r},IF({SP}$I{r}<>"",{SP}$I{r},"Comprometida y no completada al cierre del Sprint "&$N{r}),"")', XF["dattxt"])  # Motivo (editable o automático)
        ...
        d.f(r,41, f'IF($C{r},IFERROR(INDEX({CIERRES},MATCH($N{r},{SNUMS},0)),""),"")', XF["datc"])  # CierrePlan (reunión del sprint planificado)
        d.f(r,42, f'IF($C{r},IF(AND($N{r}<>9999,$AD{r}<>$N{r},ISNUMBER($AO{r})),TODAY()-$AO{r},0),"")', XF["datc"])  # DesvíoSprint (días desde el cierre)
    ...
    sheet7 = render_sheet(d, f"A1:AP{DATA_LAST}")
...
def build_guia():
    ...
    put("• Sprints: calendario de reuniones (editable) + seguimiento por tarea (Sprint planificado, vigente, desvío y motivo editable).","t",26)
